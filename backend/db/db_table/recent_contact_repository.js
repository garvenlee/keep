const dbSchema = `CREATE TABLE IF NOT EXISTS RecentContact (
...
class RecentContactRepository {
    ...
    create(user_one_id, user_two_id) {
        ...
        return this.dao.run(
            'INSERT INTO RecentContact (user_one_id, user_two_id) VALUES( ? , ? )',
            [user_one_id, user_two_id]);
    }

    delete(user_one_id, user_two_id) {
        return this.dao.run(
            `DELETE FROM RecentContact WHERE user_one_id = ? and user_two_id = ?`,
            [user_one_id, user_two_id]
        );
    }
    ...
    getContactsList(user_id) {
        return this.dao.all(
            `SELECT * FROM RecentContact WHERE user_id = ?`,
            [user_id]);
    }
}

module.exports = RecentContactRepository;